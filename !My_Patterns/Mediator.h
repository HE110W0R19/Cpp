...
using namespace std;
/*
Oбъект диалогового окна функционирует в качестве посредника.Виджеты диалогового
окна ничего не знают о своих соседях.Всякий раз, когда происходит взаимодействие
с пользователем виджет в Widget::changed() "делегирует" это событие посреднику 
mediator->widgetChanged(this).

FileSelectionDialog::widgetChanged() инкапсулирует все коллективное поведение 
для диалогового окна(служит центром взаимодействия).Пользователь может выбрать 
"взаимодействие" с полем редактирования Filter, списком Directories, списком 
Files или полем редактирования Selection.
*/
...
class FileSelectionDialog /// mediator обеспечивающий взаимодействие
{
private:
